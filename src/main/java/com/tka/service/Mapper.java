...
public class Mapper {
	...
	public static JobResponseDTO mapToJobResponseDTO(Job job) {
		...
		dto.setSkills(job.getSkills().stream().map(Mapper::mapToSkillResponseDTO).collect(Collectors.toList()));
		return dto;
	}
	...
	public static JobApplicationResponseDTO mapToJobApplicationResponseDTO(JobApplication application) {
		...
		dto.setJob(mapToJobResponseDTO(application.getJob()));
		dto.setUser(mapToUserResponseDTO(application.getUser()));
